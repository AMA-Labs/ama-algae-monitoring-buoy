...
class SegmenterProcess(multiprocessing.Process):
    ...
    def _open_and_apply_flat(self, filepath, flat_ref):
        ...
        # Correct image
        if np.size(image, 0) == np.size(self.__flat, 1):
    ...
    def _slice_image(self, img, name, mask, start_count=0):
        ...
        regionprops_filtered = [
            region for region in regionprops if (region.bbox_area >= self.minpixsize**2 and region.bbox_area <= self.maxpixsize**2)
        ]
        object_number = len(regionprops_filtered)
        logger.debug(f"Found {nlabels} labels, or {object_number} after size filtering")

        path = os.path.join(self.__working_obj_path, "output\\")
        if not os.path.exists(path):
            os.makedirs(path)
        for i in range(0, 10000):
            _path = os.path.join(path, f"{name}" + ("" if i == 0 else "_"+str(i)) + "\\")
    ...
    def _pipe(self, ecotaxa_export):
        ...
        for (i, filename) in enumerate(images_list):
            ...
            A1 = self.__debug_objects_root
            A2 = self.__working_debug_path
            A3 = self.__img_path
            A4 = name
    ...
    def run(self):
        ...
        handler = functools.partial(
            streamer.StreamingHandler, refresh_delay
        )
        try:
            server = streamer.StreamingServer(address, handler)
        except Exception as e:
            logger.exception(
